...
class CosyVoiceFrontEnd:
    ...
    def __init__(self,
        ...
        # 加载 campplus (说话人编码器) - 使用 CPU
        print("  [frontend] 加载 campplus 模型...")
        gc.collect()
        self.campplus_session = onnxruntime.InferenceSession(
            campplus_model, sess_options=option, providers=["CPUExecutionProvider"]
        )

        # 加载 speech_tokenizer - 优先 CPU 以节省 GPU 显存
        # 因为 speech_tokenizer 只在预处理时使用，不是推理瓶颈
        print("  [frontend] 加载 speech_tokenizer 模型...")
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()
        ...
        try:
            self.speech_tokenizer_session = onnxruntime.InferenceSession(
                ...
                providers=["CPUExecutionProvider"]  # 强制使用 CPU 以节省显存
            )
        except Exception as e:
            print(f"  [frontend] ONNX 加载警告: {e}")
            print("  [frontend] 尝试使用 CPU Provider...")
            self.speech_tokenizer_session = onnxruntime.InferenceSession(
                speech_tokenizer_model, sess_options=option,
                providers=["CPUExecutionProvider"]
            )

        print("  [frontend] ONNX 模型加载完成")
